...
func newMemAuthz() *memAuthz { return &memAuthz{live: map[string]proxy.Grant{}} }
...
func TestUnadvertisedToolIsUnreachableWithoutAGrant(t *testing.T) {
	r := labRecipe(t)
	g := proxy.Gate{
		Routes: proxy.Router{"lab__fix": {Recipe: r, RecipeHash: "h", RecipeName: "p",
			GateArg: "image", Sequenced: true}},
		Authorizations: newMemAuthz(),
	}
	d := g.Decide(context.Background(), proxy.ToolCall{Tool: "lab__fix", Args: map[string]string{"image": "badport"}})
	if d.Forward {
...
func TestGrantMakesTheExactCallReachable(t *testing.T) {
	...
	_ = az.Mint(context.Background(), proxy.Grant{Fingerprint: fp, Tool: "lab__fix", Source: "policy:lab_repair_badport", Session: "s"})
	...
	d := g.Decide(context.Background(), proxy.ToolCall{Tool: "lab__fix", Args: args})
	if !d.Forward {
...
func TestGrantDoesNotBypassTheRecipe(t *testing.T) {
	...
	_ = az.Mint(context.Background(), proxy.Grant{Fingerprint: proxy.Fingerprint("lab__fix", args), Tool: "lab__fix", Source: "policy:x", Session: "s"})
...
func TestGrantIsBoundToItsArguments(t *testing.T) {
	...
	_ = az.Mint(context.Background(), proxy.Grant{Fingerprint: proxy.Fingerprint("lab__fix", granted), Tool: "lab__fix", Source: "policy:x", Session: "s"})
	...
	if d := g.Decide(context.Background(), proxy.ToolCall{Tool: "lab__fix", Args: other}); d.Forward {
...
func TestGrantIsSpentOnUse(t *testing.T) {
	...
	args := map[string]string{"image": "badport"}
	fp := proxy.Fingerprint("lab__fix", args)
	_ = az.Mint(context.Background(), proxy.Grant{Fingerprint: fp, Tool: "lab__fix", Source: "policy:x", Session: "s"})
...
func TestDeniedCallDoesNotBurnTheGrant(t *testing.T) {
	...
	g.Decide(context.Background(), proxy.ToolCall{Tool: "lab__fix", Args: args})
	if len(az.burned) != 0 {
...
func TestOrdinaryRouteNeedsNoGrant(t *testing.T) {
	r := labRecipe(t)
	g := proxy.Gate{
		Routes:         proxy.Router{"lab__read": {Recipe: r, RecipeHash: "h", RecipeName: "p", GateArg: "image"}},
		Authorizations: newMemAuthz(),
	}
	d := g.Decide(context.Background(), proxy.ToolCall{Tool: "lab__read", Args: map[string]string{"image": "badport"}})
	if !d.Forward {
...
func TestNoAuthorizationStoreFailsClosed(t *testing.T) {
	r := labRecipe(t)
	g := proxy.Gate{Routes: proxy.Router{"lab__fix": {Recipe: r, RecipeHash: "h", RecipeName: "p",
		GateArg: "image", Sequenced: true}}} // no Authorizations
	if d := g.Decide(context.Background(), proxy.ToolCall{Tool: "lab__fix", Args: map[string]string{"image": "badport"}}); d.Forward {
...
func TestGrantRecordsItsMinter(t *testing.T) {
	az := newMemAuthz()
	_ = az.Mint(context.Background(), proxy.Grant{Fingerprint: "fp", Tool: "t", Source: "policy:lab_repair_badport", Session: "s"})
